...
type QuestionType = "text" | "numeric" | "single_select" | "multi_select" | "rating" | "upload";
...
export function FormFillClient({ form, questions }: { form: FormDef; questions: QuestionDef[] }) {
  ...
  return (
    <div className="min-h-screen flex items-start justify-center px-4 py-10 md:py-16" style={{ background: "var(--page-bg)" }}>
      <div className="w-full max-w-xl rounded-xl p-6 md:p-8" style={{ background: "var(--surface-bg)" }}>
        ...
        {justSubmitted ? (
          ...
          <div className="mt-6 flex flex-col gap-5">
            ...
            {activeQuestions.map((q) => (
              <div key={q.id} className="flex flex-col gap-1.5">
                ...
                {q.question_type === "text" && (
                  <textarea rows={2} value={answers[q.id]?.value_text ?? ""} onChange={(e) => updateAnswer(q.id, { value_text: e.target.value })}
            ...
            <button onClick={() => void handleSubmit()} disabled={submitting}
              className="h-10 px-5 rounded-lg text-[13.5px] font-semibold text-white disabled:opacity-60 self-start"
              style={{ background: "var(--navy)" }}>
