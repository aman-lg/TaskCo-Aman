...
// One Ask Tasko turn: reads recent history, calls Gemini with tools,
...
// directly. Shared by the messages route (triggered via after(), so it
// keeps running even if the user navigates away right after sending) and
// the dedicated ai-reply route (kept for a possible future manual-retry
// affordance).
...
export async function generateAiReply(
  ...
  try {
    for (let i = 0; i < MAX_TOOL_ITERATIONS; i++) {
      ...
      const toolResult = await executeReadTool(supabase, userId, name, args);
      contents.push(modelTurn);
  ...
  if (pendingAction) {
    ...
    if (msgErr || !message) { console.error("[ai-reply] message insert failed", msgErr); return { ok: false, error: "internal" }; }
    ...
    if (actionErr || !action) { console.error("[ai-reply] action insert failed", actionErr); return { ok: false, error: "internal" }; }
  ...
  const { data: message, error: msgErr } = await (admin as any)
    .from("messages")
    .insert({ conversation_id: conversationId, sender_id: null, type: "text", content: finalText, metadata: { is_ai: true } })
    .select("*")
    .single();
