-- ─────────────────────────────────────────────
-- Re-applying migration 016's fix — testing just now showed the exact same
-- "delete a project that has tasks in it" bug still reproduces, meaning 016
-- was likely never actually applied (no confirmation of it specifically was
-- ever obtained — the conversation moved to other topics right after it was
-- given). Idempotent either way: this is identical to 016's function body.
-- ─────────────────────────────────────────────
